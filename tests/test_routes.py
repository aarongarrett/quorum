...
def test_home_route_shows_meetings(client, db_connection):
    ...
    # Check if meeting is shown
    assert f"/checkin/{meeting_id}".encode() in response.data

    # Check date and time formatting in response
    date_str = strftime(start_time, "%B %d, %Y")
    # Use a format that works on all platforms
    start_str = strftime(start_time, "%I:%M %p")
    end_str = strftime(end_time, "%I:%M %p")

    assert date_str.encode() in response.data
    # Check for both possible time formats (with/without leading zero)
    assert (
        start_str.encode() in response.data
        or start_time.strftime("%H:%M").encode() in response.data
    )
    assert (
        end_str.encode() in response.data
        or end_time.strftime("%H:%M").encode() in response.data
    )
